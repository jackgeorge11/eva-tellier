...
export default function Info({ info, refs, openStates, handleClick }) {
    ...
    <>
      {info.map((tab, i) => (
        <div
          ...
          className={`project about ${openStates[tab.slug] && "is--open"} ${!i && "first"}`}
          onClick={
        ...
        >
          ...
          <h3 className="year">{tab.title}</h3>
        </div>
      ))}
    </>
  );
